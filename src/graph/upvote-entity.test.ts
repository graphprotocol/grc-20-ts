...
describe('upvote-entity', () => {
  ...
  describe('voteOnEntity', () => {
    ...
    it('encodes an upvote action correctly', async () => {
    ...
    it('encodes a downvote action correctly', async () => {
      const result = await voteOnEntity({
        entityId: validEntityId,
        spaceId: validSpaceId,
        voteType: 'down',
    ...
    it('encodes a remove vote action correctly', async () => {
      const result = await voteOnEntity({
        entityId: validEntityId,
        spaceId: validSpaceId,
        voteType: 'remove',
    ...
    it('throws InvalidEntityIdError for invalid entity ID', async () => {
      ...
      ).rejects.toThrow('Invalid entity ID: not-a-valid-uuid');
    });

    it('throws InvalidSpaceIdError for invalid space ID', async () => {
      ...
      ).rejects.toThrow('Invalid space ID: invalid-space-id');
    });

    it('throws InvalidVoteTypeError for invalid vote type', async () => {
      await expect(
        voteOnEntity({
          entityId: validEntityId,
          spaceId: validSpaceId,
      ...
      ).rejects.toThrow("Invalid vote type: invalid. Must be 'up', 'down', or 'remove'");
    });

    it('produces different payloads for different vote types', async () => {
      const upvoteResult = await voteOnEntity({
        entityId: validEntityId,
        spaceId: validSpaceId,
        voteType: 'up',
      });

      const downvoteResult = await voteOnEntity({
        entityId: validEntityId,
        spaceId: validSpaceId,
        voteType: 'down',
      });

      const removeResult = await voteOnEntity({
        entityId: validEntityId,
        spaceId: validSpaceId,
        voteType: 'remove',
      });
